...
export function projectOntoSegment(px: number, py: number, segment: Segment) {
  const dx = segment.bx - segment.ax;
  const dy = segment.by - segment.ay;
  const lengthSquared = dx * dx + dy * dy;
  const t = lengthSquared === 0 ? 0 : Math.min(1, Math.max(0, ((px - segment.ax) * dx + (py - segment.ay) * dy) / lengthSquared));
...
export type SegmentIndex = {
  forEachNear(minX: number, minY: number, maxX: number, maxY: number, visit: (segment: Segment) => void): void;
};

export function createSegmentIndex(segments: Segment[], cellSize = 48): SegmentIndex {
  const buckets = new Map<number, number[]>();
  const key = (cx: number, cy: number) => (cx + 8192) * 65536 + (cy + 8192);

  segments.forEach((segment, id) => {
    const minCellX = Math.floor(Math.min(segment.ax, segment.bx) / cellSize);
    const maxCellX = Math.floor(Math.max(segment.ax, segment.bx) / cellSize);
    const minCellY = Math.floor(Math.min(segment.ay, segment.by) / cellSize);
    const maxCellY = Math.floor(Math.max(segment.ay, segment.by) / cellSize);
  ...
  return {
    forEachNear(minX, minY, maxX, maxY, visit) {
      stamp += 1;
...
export function castRay(
  ...
  const endX = originX + directionX * range;
  const endY = originY + directionY * range;
  let nearest = range;
  ...
  index.forEachNear(
    ...
    (segment) => {
      ...
      const denominator = directionX * sy - directionY * sx;
      if (Math.abs(denominator) < 1e-9) return;
      const qx = segment.ax - originX;
      const qy = segment.ay - originY;
      const along = (qx * sy - qy * sx) / denominator;
      const across = (qx * directionY - qy * directionX) / denominator;
      if (along < 0 || along > nearest || across < 0 || across > 1) return;
      nearest = along;
    },
  );
