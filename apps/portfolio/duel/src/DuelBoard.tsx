...
function SciMark({ k, size = 15 }: { k: ScienceSymbol; size?: number }) {
  ...
  return (
    <svg aria-hidden="true" width={s} height={s} viewBox={`0 0 ${s} ${s}`}
...
const Shield = ({ n }: { n: number }) => (
  <span style={{ display: "inline-flex", alignItems: "center", gap: 2 }}>
    <svg aria-hidden="true" width="13" height="14" viewBox="0 0 13 14" style={{ filter: "drop-shadow(0 1px 1px rgba(0,0,0,.5))", flex: "0 0 auto" }}>
      <defs>
...
const CardBack = ({ w = 74 }: { w?: number }) => (
  <div style={{
    ...
    <svg aria-hidden="true" viewBox="0 0 40 40" width="52%" style={{
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {st.players[1 - pd.player].built.filter((id) => CARD[id].color === pd.color).map((id, k) => (
            <div
              ...
              role="button"
              tabIndex={yours ? 0 : -1}
              onClick={() => yours && commit(resolve(st, id))}
              onKeyDown={(e) => {
                if (yours && (e.key === "Enter" || e.key === " ")) {
                  e.preventDefault();
                  commit(resolve(st, id));
                }
              }}
              style={{ cursor: yours ? "pointer" : "default" }}
            >
    ...
    <Shell status={status}>
      <div style={{ maxWidth: 440, margin: "40px auto" }}>
        <div className="duel-fade" style={{ textAlign: "center", marginBottom: 30 }}>
          <svg aria-hidden="true" viewBox="0 0 64 64" width="54" height="54" style={{ opacity: .9, marginBottom: 6 }}>
