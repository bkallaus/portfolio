...
const ok=[],bad=[];
const t=(n,c,g)=>(c?ok:bad).push(n+(c?"":`  → got ${JSON.stringify(g)}`));

function rig(w0,w1=["pyramids"],tok=[],cards=1){
...
const open=st=>st.slots.filter(s=>s.card&&E.isOpen(st.slots,s))[0].id;
...
/* --- play again on the final card of an age is lost --- */
{
  ...
  st.remaining=1;
  const a=E.actWonder(st,last.id,"sphinx");
...
/* --- play again mid-age lets the same player act twice --- */
{
  ...
  const a=E.actWonder(st,open(st),"sphinx");
  t("after Sphinx it is still A's turn", a.turn===0, a.turn);
  const b=E.actWonder(a,open(a),"colossus");
...
/* --- Economy captures wonder trade spending --- */
{
  ...
  const a=E.actWonder(st,open(st),"pyramids");
...
/* --- destroy interrupt on the final card of an age --- */
{
  ...
  const last=st.slots[st.slots.length-1]; last.card="lumber_yard"; last.up=true;
  st.remaining=1;
  const a=E.actWonder(st,last.id,"circus_maximus");
...
/* --- a wonder that both wins militarily and has a destroy effect --- */
{
  ...
  const a=E.actWonder(st,open(st),"circus_maximus");
...
/* --- looting tokens fire once, in the right direction --- */
{
  ...
  const a=E.actWonder(st,open(st),"colossus");   // → +4, crosses 3
...
/* --- wonder production is usable immediately --- */
{
  ...
  const a=E.actWonder(st,open(st),"great_lighthouse");
  ...
  const a2=E.actWonder(st,open(st),"great_lighthouse");
