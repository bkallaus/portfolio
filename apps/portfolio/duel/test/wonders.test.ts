...
const t = (name: string, cond: unknown, got?: unknown) => (cond ? ok : bad).push(name + (cond ? "" : `  → got ${JSON.stringify(got)}`));
...
const must = (st: E.GameState | null): E.GameState => {
  if (!st) throw new Error("expected a legal move");
  return st;
};

/* cardCost only ever reaches into players, so a stub carrying just that is
   enough to price a card against a hand-made pair of cities. */
...
const openSlot = (st: E.GameState): number => st.slots.filter((s) => s.card && E.isOpen(st.slots, s))[0].id;
...
for (const id of ["sphinx", "hanging_gardens", "temple_of_artemis", "appian_way", "piraeus"]) {
  const st = rig([id]);
  const a = must(E.actWonder(st, openSlot(st), id));
  t(`${E.WON[id].name} grants another turn`, a.turn === 0, a.turn);
}
{
  const st = rig(["colossus"]);
  const a = must(E.actWonder(st, openSlot(st), "colossus"));
...
/* ---------- 4. coin and treasury effects ---------- */
{
  ...
  const a = must(E.actWonder(st, openSlot(st), "temple_of_artemis"));
...
}
{
  ...
  const a = must(E.actWonder(st, openSlot(st), "appian_way"));
...
/* ---------- 6. production ---------- */
{
  ...
  const a = must(E.actWonder(st, openSlot(st), "great_lighthouse"));
  const v = E.view(a, 0);
  t("Great Lighthouse adds a flexible producer", v.choices.length === 1 && v.choices[0].length === 3, v.choices);
  const vsLighthouse = twoCities([{ coins: 99, built: [], wonders: [], tokens: [] }, a.players[0]]);
  t("Great Lighthouse does not inflate opponent trade price",
    E.cardCost(vsLighthouse, 0, E.CARD.baths).total === 2,
    E.cardCost(vsLighthouse, 0, E.CARD.baths).total);
}
...
/* ---------- 8. interrupts ---------- */
{
  ...
  const a = must(E.actWonder(st, openSlot(st), "circus_maximus"));
...
}
{
  ...
  const a = must(E.actWonder(st, openSlot(st), "statue_of_zeus"));
  t("Statue of Zeus targets brown", a.pending && a.pending.type === "destroy" && a.pending.color === "brown", a.pending);
}
{
  const st = rig(["mausoleum"]);
  st.discard = ["altar", "quarry"];
  const a = must(E.actWonder(st, openSlot(st), "mausoleum"));
...
}
{
  ...
  const a = must(E.actWonder(st, openSlot(st), "great_library"));
  const pd = a.pending;
  const options = pd && pd.type === "library" ? pd.options : [];
  t("Great Library offers 3 archived tokens",
    pd && pd.type === "library" && options.length === 3, pd);
  t("Library options come from the box, not the board",
    options.every((o) => st.box.includes(o)), options);
  const b = E.resolve(a, options[0]);
...
/* ---------- 10. the seven wonder cap ---------- */
{
  ...
  const seq: [number, string][] = [[0, "colossus"], [1, "great_lighthouse"], [0, "pyramids"], [1, "piraeus"],
               [0, "sphinx"], [1, "hanging_gardens"], [0, "mausoleum"]];
  for (const [p, id] of seq) {
    st.turn = p; st.players[p].coins = 99; st.pending = null;
    st = must(E.actWonder(st, openSlot(st), id));
    const pd = st.pending;
    if (pd) st = E.resolve(st, pd.type === "mausoleum" ? st.discard[0] : pd.type === "library" ? pd.options[0] : 0);
  }
...
test("wonder scoring and costs hold", () => {
  expect(bad).toEqual([]);
});
